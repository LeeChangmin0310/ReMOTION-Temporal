...
_C.TRAIN.DATA.LABEL_SOURCE = 'PPG'
_C.TRAIN.DATA.SPLIT_METHOD = ''   # or "KFold" (if NaN -> split(8/2))
_C.TRAIN.DATA.NUM_FOLDS = 5
_C.TRAIN.DATA.FOLD_INDEX = 0
_C.TRAIN.DATA.LABEL_COLUMN = ''
_C.TRAIN.DATA.current_subject = None
_C.TRAIN.DATA.DATASET = ''
_C.TRAIN.DATA.DO_PREPROCESS = False
_C.TRAIN.DATA.DATA_FORMAT = 'NDCHW'
_C.TRAIN.DATA.BEGIN = 0.0
_C.TRAIN.DATA.END = 0.8
...
_C.VALID.DATA.LABEL_SOURCE = 'PPG'
_C.VALID.DATA.LABEL_COLUMN = ''
_C.VALID.DATA.SPLIT_METHOD = ''   # or "KFold" (if NaN -> split(8/2))
_C.VALID.DATA.NUM_FOLDS = 5
_C.VALID.DATA.FOLD_INDEX = 0
_C.VALID.DATA.current_subject = None
...
_C.TEST.DATA.LABEL_SOURCE = 'PPG'
_C.TEST.DATA.SPLIT_METHOD = ''   # or "KFold" (if NaN -> split(8/2))
_C.TEST.DATA.NUM_FOLDS = 5
_C.TEST.DATA.FOLD_INDEX = 0
_C.TEST.DATA.LABEL_COLUMN = ''
_C.TEST.DATA.current_subject = None
...
_C.UNSUPERVISED.DATA.LABEL_SOURCE = 'PPG'
_C.UNSUPERVISED.DATA.SPLIT_METHOD = ''   # or "KFold" (if NaN -> split(8/2))
_C.UNSUPERVISED.DATA.NUM_FOLDS = 5
_C.UNSUPERVISED.DATA.FOLD_INDEX = 0
_C.UNSUPERVISED.DATA.LABEL_COLUMN = ''
_C.UNSUPERVISED.DATA.current_subject = None
...
_C.ENCODER.RESUME = ''
# Dropout rate
_C.ENCODER.DROP_RATE = 0.0
_C.ENCODER.MODEL_DIR = 'PreTrainedModels'

# -----------------------------------------------------------------------------
# Model Settings for PhysMamba
# -----------------------------------------------------------------------------
# New configuration for our emotion recognition pipeline
_C.MODEL.EMOTION = CN()
# Use pretrained PhysMamba as rPPG encoder
# _C.MODEL.EMOTION.USE_PHYSMAMBA = True
# rPPG signal length per chunk (output dimension from PhysMamba)
# _C.MODEL.EMOTION.RPPG_SIGNAL_LENGTH = 128
# Embedding dimensions for each branch
_C.MODEL.EMOTION.TEMPORAL_EMBED_DIM = 256
_C.MODEL.EMOTION.PRV_EMBED_DIM = 128
_C.MODEL.EMOTION.CWFREQ_EMBED_DIM = 128
# Fusion dimension (if concatenating branch embeddings)
_C.MODEL.EMOTION.FUSION_DIM = _C.MODEL.EMOTION.TEMPORAL_EMBED_DIM + _C.MODEL.EMOTION.PRV_EMBED_DIM + _C.MODEL.EMOTION.CWFREQ_EMBED_DIM

# -----------------------------------------------------------------------------
# Model Settings for physnet parameters
# -----------------------------------------------------------------------------
_C.MODEL.PHYSNET = CN()
...
def update_config(config, args):
    ...
    if config.ReMOTION_MODE == 'unsupervised_method' and config.UNSUPERVISED.DATA.PREPROCESS.USE_PSUEDO_PPG_LABEL == True:
    ...
    if config.ReMOTION_MODE == 'train_and_test' or config.ReMOTION_MODE == 'only_test':
        ...
        raise ValueError('ReMOTION_MODE only supports train_and_test, only_test, or unsupervised_method!')
